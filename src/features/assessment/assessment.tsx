...
export function Assessment() {
  ...
  return <section className="assessment-section" id="autodiagnostico"><div className="container assessment-layout"><div className="assessment-intro"><p className="eyebrow">Un punto de partida</p><h2>¿Dónde puede<br />operar mejor<br />tu empresa?</h2><p>Ocho preguntas para mirar tu operación con otros ojos.</p><span className="assessment-label">Sin registro · A tu ritmo</span><p className="small-note">Es una orientación inicial. No reemplaza un diagnóstico con el equipo.</p></div><div className="assessment-panel">
    {!started ? <div className="assessment-welcome"><span className="assessment-glyph" aria-hidden="true">↗</span><h3>Haz espacio para la siguiente mejora.</h3><p>Revisa procesos, información y herramientas. Al terminar, encontrarás oportunidades para conversar.</p><button className="button button-primary" onClick={() => { setStarted(true); track('assessment_start'); focusHeading(); }}>Iniciar autodiagnóstico<Arrow /></button><p className="small-note">Tus respuestas se quedan en esta pestaña. Sólo se envían si decides adjuntarlas al solicitar contacto.</p><noscript>Activa JavaScript para responder el autodiagnóstico.</noscript></div>
    : result ? <div className="assessment-result"><p className="eyebrow">Tu orientación inicial</p><h3 ref={heading} tabIndex={-1}>{result.title}</h3><p>{result.description}</p>{result.opportunities.length > 0 && <ul>{result.opportunities.map(item => <li key={item}>{item}</li>)}</ul>}<h4>Hablemos sobre estas oportunidades.</h4><Cta href={`/diagnostico?interes=${result.area}`}>Solicitar diagnóstico</Cta><button className="text-button" onClick={() => { setAnswers([]); setStep(0); setComplete(false); assessmentTransfer.clear(); focusHeading(); }}>Volver a responder</button></div>
