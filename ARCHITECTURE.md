...
Runtime: next/react/react-dom (framework), nodemailer (SMTP seguro). Build: TypeScript/types, ESLint 10 con @eslint/js + @next/eslint-plugin-next + typescript-eslint + react-hooks compatibles, Tailwind/PostCSS. QA: Playwright (journeys/responsive), axe (accesibilidad), Lighthouse (laboratorio), tsx (pruebas TypeScript con runner nativo). No UI kit, state manager, icon pack, ORM ni SDK de IA/CRM. Versiones exactas en package.json/lockfile; auditoría en docs/audits/npm-audit.json. Se evita eslint-config-next porque sus plugins React/a11y aún limitan ESLint a 9 fuera de soporte; axe cubre accesibilidad real en navegador.
